...
async function main() {
  const category = process.env.SANTA_CATEGORY || 'lacteos-huevos-y-congelados/leches/leche-liquida'
  ...
  const santa = page.getByText('Santa Isabel', { exact: true }).locator('..')
  await santa.getByRole('checkbox').check()
  await santa.locator('..').getByRole('combobox').selectOption(category)
