...
- Falabella: `tottus`. Usa categorias reales curadas con IDs/rutas `CATG...`, por ejemplo `CATG27055/Despensa`. Si existe `TOTTUS_API_TEMPLATE`, consulta ese endpoint JSON; de lo contrario abre la PLP con Playwright y lee los primeros 48 productos SSR desde `__NEXT_DATA__` (el SSR ignora `?page=2`). Tottus bloquea requests directos y Chromium headless con Cloudflare, por lo que el fallback usa Chrome visible por defecto (`TOTTUS_BROWSER_CHANNEL=chrome`, `TOTTUS_BROWSER_HEADLESS=0`). La ruta `/tottus-cl/search?Ntt=<termino>` puede responder 526; en ese caso busca localmente el termino sobre las PLP de categorias y deja un warning en `meta`.
