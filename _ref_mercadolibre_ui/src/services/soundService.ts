...
class SoundService {
  ...
  // Refined for RE-style "Click/Select" - Layered tick + thud
  playClick() {
    if (!this.enabled) return;
    this.initCtx();
    if (!this.ctx) return;
    ...
    // Tapping sound component (High frequency tick)
  ...
  // Refined for RE-style "Inventory Open" - Resonant filter sweep
  playOpen() {
    if (!this.enabled) return;
    this.initCtx();
    if (!this.ctx) return;
  ...
  playBeep(freq = 440, duration = 0.1) {
    ...
    osc.type = 'square'; // More "retro" than sine
