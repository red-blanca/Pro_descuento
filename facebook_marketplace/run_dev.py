...
def main() -> int:
    ...
    web_cmd = [str(node_exe), str(vite_js), "dev", "--host", "127.0.0.1", "--port", str(WEB_PORT)]

    api = None
    if _is_port_open("127.0.0.1", API_PORT):
        print(f"API ya activa en http://127.0.0.1:{API_PORT} (se reutiliza).")
    else:
        print(f"Iniciando API en http://127.0.0.1:{API_PORT}")
    ...
    print(f"Iniciando Frontend en http://127.0.0.1:{WEB_PORT}")
